...
```sh
./target/release/substrate-task-auction -h
```
...
```bash
./target/release/substrate-task-auction --dev
```

Purge the development chain's state:

```bash
./target/release/substrate-task-auction purge-chain --dev
```

Start the development chain with detailed logging:

```bash
RUST_BACKTRACE=1 ./target/release/substrate-task-auction -ldebug --dev
```
...
$ ./target/release/substrate-task-auction --dev --base-path ./my-chain-state/
...
```shell
./target/release/substrate-task-auction --help
```
...
(`cargo build --release && ./target/release/substrate-task-auction --dev --ws-external`)
by appending your own. A few useful ones are as follow.

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/substrate-task-auction --dev --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/substrate-task-auction purge-chain --dev
